    ...
    public class ContextSolver
    {
        ...
        private void GetSteering()
        {
            foreach (Collider2D obstacleCollider in _pawn.Sensor.DetectedObstacles)
            {
                ...
                _weight = _directionToObstacle.magnitude <= _pawn.Collider.radius ? 1f : (_pawn.Status.StatHolder.ViewDistance - _directionToObstacle.magnitude) / _pawn.Status.StatHolder.ViewDistance;
